前置知识：[分块](https://www.cnblogs.com/RioTian/p/15113195.html)。
...
<details open="" class="warn"> <summary> Sample Description </summary>  <p>规定 $f(x)$ 为整数 $x$ 的二进制表示中 $1$ 的个数。输入一个正整数 $n$($n\leq 10^9$)，输出 $\sum_{i=1}^n f^2(i)$。 </p> </details><br>

如果对于每一个 $n$，都输出 $f(n)$ 的话，除了可能会 MLE 外，还有可能代码超过最大代码长度限制，导致编译不通过。

我们考虑优化这个答案表。采用 [分块](https://www.cnblogs.com/RioTian/p/15113195.html) 的思想，我们设置一个合理的步长 $m$（这个步长一般视代码长度而定），对于第 $i$​ 块，计算出：
...
<details open="" class="warn"> <summary> 注意事项 </summary>  <p>当上题中指数不是定值，但是范围较小，也可以考虑打表。 </p> </details><br>
